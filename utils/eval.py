...
def eval_ppl(model, tokenizer, seed=0, nsamples=32, device=torch.device("cuda:0"), dataset="wikitext2", bsz=1):

	# Print status
    print(f"evaluating on {dataset}")
    if dataset == 'bookcorpus': seqlen = 128
    else: seqlen = model.seqlen
	# Get the test loader
    trainloader, testloader = get_loaders(
		dataset, nsamples=nsamples, seed=seed, seqlen=seqlen, tokenizer=tokenizer 
	)

	# Evaluate ppl in no grad context to avoid updating the model
    with torch.no_grad():
        ppl_test = eval_ppl_test(model, testloader, bsz, device, seqlen)
        ppl_train = eval_ppl_train(model, trainloader, bsz, device, seqlen)
...
def eval_ppl_train(model, trainloader, bs=1, device=None, seqlen=128):
	# Get input IDs
...
# Function to evaluate perplexity (ppl) specifically on the wikitext dataset
def eval_ppl_test(model, testenc, bs=1, device=None, seqlen=128):
	# Get input IDs
	testenc = testenc.input_ids

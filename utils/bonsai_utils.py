...
def investigate_score_based_mask(args, model, wandb_run, epoch_=1):
	...
	for (name, module) in model.named_modules():
		...
		if name.endswith('mlp') or name.endswith('self_attn'):
			...
			module.bonsai_prune_method = args.bonsai_prune_method
	...
	# Revert to a forward pass with batch size of 1
	try:
		eval_ppl_trainonly(model, tokenizer, bsz=args.bsz, nsamples=args.nsamples, dataset=args.calib_dataset)
	except Exception as e:
		print(e)
		gc.collect()
		torch.cuda.empty_cache()
		eval_ppl_trainonly(model, tokenizer, bsz=1, nsamples=args.nsamples, dataset=args.calib_dataset)
...
def args_to_dict(args):
	...
	return {
		...
		'pmethod': args.bonsai_prune_method,
...
def main():
	...
	_, orig_test_ppl = eval_ppl(model, tokenizer, model.device, dataset=args.calib_dataset)
